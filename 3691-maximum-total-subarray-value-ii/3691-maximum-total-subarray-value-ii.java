class Solution {
    public long maxTotalValue(int[] nums, int k) {
        ...
        PriorityQueue<int[]> pq = new PriorityQueue<>((num,b) -> b[0]-num[0]);  //creates descesding order heap

        for(int i =0 ; i<n ; i++)
            pq.add(new int[] {LUT.query(i,n) ,i ,n});  //i and n are indexes showing the range 
        long res =0;
        while(pq.peek() [0]>0){
            int[] top = pq.poll();
            res += top[0];
